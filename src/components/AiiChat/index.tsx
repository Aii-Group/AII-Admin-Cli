...
const MarkdownComponents: Components = {
  ...
  a({ href, children, ...props }) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className="primary-text-btn hover:underline" {...props}>
        {children}
      </a>
    )
  },
    ...
    <>
      {!['local'].includes(status) && message.thinkingContent && (
        <>
          ...
          <div className={`think-wrapper-${id} border-l-2 text-light-colorFill border-light-colorPrimary pl-10 my-10`}>
...
const AiiChat: React.FC = () => {
  ...
  return (
    <div className="aii-chat">
      <Bubble.List style={{ maxHeight: 'calc(100% - 160px)', overflowY: 'auto' }} roles={roles} items={bubbleItems} />
      <Sender
        className="sender"
        classNames={{
        ...
        actions={(_, info) => {
          ...
          return (
            <div className="w-full flex justify-between items-center">
              ...
              {agent.isRequesting() ? <LoadingButton type="default" /> : <SendButton type="primary" icon={<Send />} />}
            </div>
          )
        }}
      />
